...
for (chrom in 1:22){
    ...
    if (!is.na(opt$afreq)){
        cl_args <- c(cl_args, c('--read-freq', paste0(opt$afreq, chrom, '.afreq')))    
    }
    
    # cat('plink2',paste(cl_args, collase=' '), '\n')
    
    called <- system2('plink2', args = cl_args, stdout = T, stderr = T)
...
fwrite(profiles, file = paste0(opt$out,'.profiles.tsv.gz'), sep="\t")
cat(paste0('Wrote predictions to ',opt$out,'.profiles.tsv.gz\n'))
