...
for(i in chrom){
  ...
  frq<-frq[frq$MAF > 0.0,]
  write.table(frq$SNP,paste0(Geno_1KG_dir,'/LD_matrix/',popul,'/chr',i,'/SNP.txt'), col.names=F, row.names=F, quote=F)
  nsnp <- nrow(frq)
  if (nsnp < chunksize){
    nsnp_chunk <- 1
  } else {
  ...
  for(j in 1:nsnp_chunk){
    ...
    # print(start)
    # print(end)
    outfile <- paste0(Geno_1KG_dir,'/LD_matrix/',popul,'/chr',i,'/SNP_snp',start,'-',end,'.txt')
    sink(outfile)
    cat(start,' ',end,'\n')
    sink()
  }
