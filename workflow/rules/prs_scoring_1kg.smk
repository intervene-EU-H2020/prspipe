...
rule prs_scoring_ptc_nested_1kg_ancestry:
    input:
        genotypes = expand('resources/1kg/1KGPhase3.w_hm3.chr{chrom}.{ext}', chrom=range(1,23), ext=['bed','bim','fam']),
        keep = 'resources/1kg/keep_files/{ancestry}_samples.keep',
        score = 'prs/pt_clump/{study}/nested/1KGPhase3.w_hm3.{study}.score.gz',
        afreq = expand('resources/1kg/freq_files/{ancestry}/1KGPhase3.w_hm3.{ancestry}.chr{chrom}.afreq', chrom=range(1,23), allow_missing=True)
    output:
        full = 'prs/pt_clump_nested/{study}/1KGPhase3.w_hm3.{ancestry}.{study}.profiles.tsv.gz',
        hla = 'prs/pt_clump_nested/{study}/1KGPhase3.w_hm3.{ancestry}.{study}.hla_profiles.tsv.gz'
    log:
        'logs/prs_scoring_1kg_ancestry/{study}/{ancestry}/pt_clump_nested.log'
    resources:
        threads=1,
        mem_mb=16000,
        partition='cpu',
    ...
    shell:
         ...
         "--score {input[score]} "
         "--afreq resources/1kg/freq_files/{wildcards[ancestry]}/1KGPhase3.w_hm3.{wildcards[ancestry]}.chr "
         "--out prs/pt_clump_nested/{wildcards[study]}/1KGPhase3.w_hm3.{wildcards[ancestry]}.{wildcards[study]} "
         "--mem {resources[mem_mb]} "
         "--plink2 bin/plink2 && "
         "Rscript workflow/scripts/R/simple_per_chromosome_scorer.R "
         "--target_chr resources/1kg/1KGPhase3.w_hm3.chr "
...
rule all_prs_scoring_ptc_nested_1kg_ancestry_superpop:
    input:
        expand(rules.prs_scoring_ptc_nested_1kg_ancestry.output, study=studies['study_id'], ancestry=config['1kg_superpop'])
        
        
localrules:
    all_prs_scoring_ptc_nested_1kg_ancestry_superpop